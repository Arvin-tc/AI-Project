...
class StudentAI():
    ...
    def RecursionMove(self, counter, ansList):
        # stop the recursion and Calculate the wining rate when the simulation stop
        if (counter == 400):
            ans = ansList[0]
            for node in ansList:
            ...
            # Check if finished the simulation of the first layer
            if len(ansList) != 0:
                for ans in ansList:
                    # for the line below, maybe the range should be (0, len(Pmoves)-1) ?
                    for i in range(0,len(Pmoves)-1):
